...
namespace d {
  ...
  u32
  Context::RegisterResource(const ComPtr<ID3D12Resource> &resource, const ComPtr<D3D12MA::Allocation> &allocation) {
    ...
    u32 handle = found_spot ? spot : res_lib.resources.size();

    if (!found_spot) res_lib.resources.push_back(resource), res_lib.allocations.push_back(allocation);
    else res_lib.resources[spot] = resource, res_lib.allocations[spot] = allocation;
  ...
  Resource<Buffer> Context::create_buffer(BufferCreateInfo &&create_info) {
    ...
    DX_CHECK(allocator->CreateResource(
      &allocation_desc,
      &resource_desc,
      state,
      nullptr,
  ...
  Resource<D2> Context::create_texture_2d(TextureCreateInfo &&texture_info) {
    ...
    desc = CD3DX12_RESOURCE_DESC::Tex2D(texture_info.format, texture_info.extent.width, texture_info.extent.height,
                                        texture_info.extent.array_size, 0, 1, 0, res_flags);
    ...
    if (buffer_usage == BufferUsage::SHADER_READ) {
      return D3D12_SHADER_RESOURCE_VIEW_DESC{
        ...
        .Buffer = D3D12_BUFFER_SRV{
          ...
          .Flags = stride ? D3D12_BUFFER_SRV_FLAG_NONE : D3D12_BUFFER_SRV_FLAG_RAW,
        },
      };
      ...
      return D3D12_UNORDERED_ACCESS_VIEW_DESC{
        ...
        .Buffer = D3D12_BUFFER_UAV{
          ...
          .Flags = stride ? D3D12_BUFFER_UAV_FLAG_NONE : D3D12_BUFFER_UAV_FLAG_RAW,
        },
      };
    }
  }

  std::variant<D3D12_SHADER_RESOURCE_VIEW_DESC, D3D12_UNORDERED_ACCESS_VIEW_DESC, D3D12_RENDER_TARGET_VIEW_DESC, D3D12_DEPTH_STENCIL_VIEW_DESC>
  TextureViewInfo::get_native_view() const {
    if (type == ResourceType::D2) {
      ...
      } else if (texture_usage == TextureUsage::SHADER_READ_WRITE_ATOMIC) { // atomic read_write
        return D3D12_UNORDERED_ACCESS_VIEW_DESC{
          .Format = format,
      ...
      assert(0 && "unexpected resource type");
    }
  }
  ...
  [[nodiscard]] auto ResourceViewInfo::desc_index() const -> u32 {
    ...
    if (type == ResourceType::Buffer) {
      return lib.buffer_view_cache.contains(views.buffer_view) ? lib.storage.bindable_desc_heap.get_index_of(
        lib.buffer_view_cache[views.buffer_view]) : lib.storage.bindable_desc_heap.push_back(*this);
    }
    return lib.texture_view_cache.contains(views.texture_view) ? lib.storage.bindable_desc_heap.get_index_of(
      lib.texture_view_cache[views.texture_view]) : lib.storage.bindable_desc_heap.push_back(*this);
  }
  ...
  [[nodiscard]] Resource<D2> util::create_texture_from_file(const char *path) {
    ...
    if (texture_path.extension() == ".dds") {
      DX_CHECK(LoadFromDDSFile(texture_path.c_str(), DDS_FLAGS_FORCE_RGB, &metadata, scratch));
      ...
      DX_CHECK(LoadFromWICFile(texture_path.c_str(), WIC_FLAGS_FORCE_RGB, &metadata, scratch));
    }
    Resource<D2> res = c.create_texture_2d(TextureCreateInfo {
      .format = DXGI_FORMAT_R8G8B8A8_UNORM,
      .dim = TextureDimension::D2,
      .extent = TextureExtent {.width = u32(metadata.width), .height = u32(metadata.height)},
    ...
    UINT64 requiredSize = GetRequiredIntermediateSize( get_native_res(res), 0, (u32) subresources.size());
