...
namespace d {
  struct DescHeap {
    ...
    auto push_back_get_handle(const ResourceViewInfo& info) -> D3D12_CPU_DESCRIPTOR_HANDLE;
  ...
  struct ResourceLibrary {
    ...
    std::unordered_map<BufferViewInfo, D3D12_CPU_DESCRIPTOR_HANDLE> buffer_view_cache;
    std::unordered_map<TextureViewInfo, D3D12_CPU_DESCRIPTOR_HANDLE> texture_view_cache;

    DescriptorStorage storage;

    [[nodiscard]] auto get_resource_index(const ResourceViewInfo& info) -> u32;
    [[nodiscard]] auto get_desc_handle(const ResourceViewInfo &info) -> D3D12_CPU_DESCRIPTOR_HANDLE;
  };
  ...
  struct Context {
    ...
    [[nodiscard]] std::pair<D3D12_CPU_DESCRIPTOR_HANDLE, CommandList &> BeginRendering();

    void EndRendering();

    [[nodiscard]] Resource<Buffer> create_buffer(BufferCreateInfo &&create_info);

    [[nodiscard]] Resource<D2> create_texture_2d(TextureCreateInfo &&texture_info);

    u32 RegisterResource(const ComPtr<ID3D12Resource> &resource, const ComPtr<D3D12MA::Allocation> &allocation);
